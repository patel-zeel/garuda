...
@jaxtyped(typechecker=beartype)
def obb_labels_to_geojson(
    labels: Sequence[BB], save_path: str = None, overwrite: bool = False, source: str = None, task_name: str = None
):
    """
    ...
    """
    geojson_data = geojson.FeatureCollection([label.to_geojson(source=source, task_name=task_name) for label in labels])
...
def deduplicate(labels: Sequence[BB], ioa_threshold: float, iou_threshold: float) -> Tuple[Sequence[BB], dict, dict]:
    ...
    for master_i in range(len(labels)):
        ...
        n_after_removed = len(removed_indices)
        logger.info(
            f"Number of bounding boxes after removing overlapping bounding boxes: {len(labels) - len(removed_indices)}"
        )
        if n_after_removed == n_current_removed:
            logger.info("No more bounding boxes to remove")
            break
